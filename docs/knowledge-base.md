...
- Verify OTP response includes onboarding flags. `required_name` is true when `foundUser.Name == ""` and the client should prompt via `PATCH /api/v1/user/me`. `has_showrooms` is true when the verified user has at least one active/non-deleted showroom membership. `has_vehicles` is true when any non-deleted vehicle is assigned to a showroom the verified user belongs to, regardless of sold status. `VerifyOTPResponse` is a separate DTO from `TokenResponse` (refresh-token response).
