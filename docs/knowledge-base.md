...
- **Initial vehicle status on creation**: When a vehicle is created via `POST /api/v1/vehicle`, a `vehicle_statuses` entry with `status = bought` is automatically inserted atomically in the same DB transaction. The `added_by` field is set to the authenticated user's ID. The `bought` value was added to the `vehicle_status` enum via migration `000018`. The service calls `repo.CreateWithInitialStatus(ctx, vehicle, status)` which wraps both the vehicle insert and the status insert in a single `database.RunInTx` transaction; if the status insert fails, the vehicle insert is rolled back. The handler extracts `userID` from the gin context and passes it explicitly to `service.CreateVehicle(ctx, req, userID)`.
